...
use sandbox::environment::{has_env, is_env_on, key};
...
fn main() {
    ...
    if !is_env_on(key::SANDBOX_TESTING) && has_env(key::SANDBOX_ACTIVE) {
...
fn setup_sandbox(interactive: bool) -> Result<SandboxInfo, SetupSandboxError> {
    let work_dir = if !has_env(key::PORTAGE_TMPDIR) {
        let work_dir = env::current_dir().map_err(SetupSandboxError::GetCurrentDir)?;
        if interactive {
            env::set_var(key::SANDBOX_WORKDIR, &work_dir)
        }
        Some(work_dir)
    } else {
        None
...
fn get_tmp_dir() -> Result<PathBuf, SetupSandboxError> {
    fs::canonicalize(env::var_os(key::TMPDIR).unwrap_or(TMPDIR.into()))
